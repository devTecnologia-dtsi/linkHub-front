...
import {
  ColumnItemEnviroment,
  DatatableEnviroment,
  FormEnviroment,
} from '../../../interface/enviroment.interface';
...
export class EnviromentComponent {
  @ViewChild('drawerEnviroment', { static: false })
  drawerTemplate?: TemplateRef<drawerTemplate>;
  @ViewChild('footerEnviroment', { static: false })
  ...
  validateForm: FormGroup<FormEnviroment> = this.fb.group({
  ...
  listOfColumns: ColumnItemEnviroment[] = [
    {
      name: 'id',
      sortOrder: null,
      sortFn: (a: DatatableEnviroment, b: DatatableEnviroment) =>
  ...
  getData(): void {
    this.isLoading = true;
    this.serviceEnviroment.getAllEnviroments().subscribe((data: any) => {
  ...
  changeState(dataChange: DatatableEnviroment): void {
    ...
    this.serviceEnviroment.changeState(dataChange.id).subscribe((data: any) => {
      ...
      if (data.ok != undefined && data.ok == false) {
        this.message.error(
          '<b>¡Ups!</b> Hubo un error al actualizar el ambiente',
          { nzDuration: 2500 }
        );
    ...
    if (this.edit) {
      ...
      this.serviceEnviroment
        ...
        .subscribe((data: any) => {
          ...
          if (data.ok != undefined && data.ok == false) {
            this.message.error(
              '<b>¡Ups!</b> Hubo un error al editar el ambiente',
              { nzDuration: 2500 }
            );
            return;
          }
          ...
          this.message.success(
            '<b>¡Excelente!</b> Se actualizo el ambiente con éxito',
            { nzDuration: 2500 }
          );
      ...
      this.isSaving = true;
      this.serviceEnviroment
        .saveEnviroment(descripcion)
        .subscribe((data: any) => {
          this.isSaving = false;
          if (data.ok != undefined && data.ok == false) {
            this.message.error(
              '<b>¡Ups!</b> Hubo un error al guardar el ambiente',
              { nzDuration: 2500 }
            );
            return;
          }
          ...
          this.message.success(
            '<b>¡Excelente!</b> Se guardo el ambiente con éxito',
            { nzDuration: 2500 }
          );
          this.getData();
          this.drawerRef.close();
  ...
  close(): void {
    this.drawerRef.close();
    this.dataForm = this.globalService.createDefaultObject<DatatableEnviroment>(
      this.dataForm
    );
  }
  ...
  search(): void {
    this.listOfData = this.listOfdataCopy.filter(
      ({ descripcion }: DatatableEnviroment) => {
        return descripcion
          .toLocaleLowerCase()
