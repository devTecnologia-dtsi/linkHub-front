...
export class DocumentComponent {
  @ViewChild('drawerDocument', { static: false })
  drawerTemplate?: TemplateRef<drawerTemplate>;
  @ViewChild('footerDocument', { static: false })
  ...
  validateForm: FormGroup<FormDocument> = this.fb.group({
  ...
  listOfColumns: ColumnItemDocument[] = [
    {
      name: 'id',
      sortOrder: null,
      sortFn: (a: DatatableDocument, b: DatatableDocument) =>
  ...
  getData(): void {
    this.isLoading = true;
    this.serviceDocument.getAllDocuments().subscribe((data: any) => {
      this.isLoading = false;
      if (data.ok != undefined && data.ok == false) {
        this.message.error('¡Ups! Hubo un error al obtener los ambientes', {
          nzDuration: 2500,
        });
        return;
      }
  ...
  changeState(dataChange: DatatableDocument): void {
    ...
    this.serviceDocument.changeState(dataChange.id).subscribe((data: any) => {
      ...
      if (data.ok != undefined && data.ok == false) {
        this.message.error(
          '<b>¡Ups!</b> Hubo un error al eliminar el documento',
          { nzDuration: 2500 }
        );
  ...
  submitForm(): void {
    ...
    if (this.edit) {
      ...
      this.dataForm = {
        ...this.dataForm,
        nombre_documento: descripcion ? descripcion : '',
      };
      this.serviceDocument
        ...
        .subscribe((data: any) => {
          ...
          if (data.ok != undefined && data.ok == false) {
            this.message.error(
              '<b>¡Ups!</b> Hubo un error al editar el documento',
              { nzDuration: 2500 }
            );
            return;
          }
          ...
          this.message.success(
            '<b>¡Excelente!</b> Se actulizo el documento con exito',
            { nzDuration: 2500 }
          );
          ...
          this.listOfData = this.listOfData.map((data) => {
            return data.id == this.dataForm.id
              ? { ...data, nombre_documento: this.dataForm.nombre_documento }
              : data;
          });
          this.dataForm = {
            id: '',
      ...
      this.isSaving = true;
      this.serviceDocument.saveDocument(descripcion).subscribe((data: any) => {
        this.isSaving = false;
        if (data.ok != undefined && data.ok == false) {
          this.message.error(
            '<b>¡Ups!</b> Hubo un error al guardar el documento',
            { nzDuration: 2500 }
          );
          return;
        }
        ...
        this.message.success(
          '<b>¡Excelente!</b> Se guardo el documento con exito',
          { nzDuration: 2500 }
        );
        this.getData();
        this.drawerRef.close();
  ...
  close(): void {
    this.drawerRef.close();
    this.dataForm = this.globalService.createDefaultObject<DatatableDocument>(
      this.dataForm
    );
  }
  ...
  search(): void {
    this.listOfData = this.listOfDataCopy.filter(
      ({ nombre_documento }: DatatableDocument) => {
