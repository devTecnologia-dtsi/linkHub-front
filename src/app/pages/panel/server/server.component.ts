...
export class ServerComponent {
  @ViewChild('drawerServer', { static: false })
  drawerTemplate?: TemplateRef<drawerTemplate>;
  @ViewChild('footerServer', { static: false })
  ...
  listOfColumns: ColumnItemServer[] = [
    ...
    },
    {
      ...
      sortOrder: null,
      sortFn: (a: DatatableServer, b: DatatableServer) =>
        a.nombre_sistema_operativo.localeCompare(b.nombre_sistema_operativo),
      showSort: true,
    },
    {
      name: 'Ambiente',
      sortOrder: null,
      sortFn: (a: DatatableServer, b: DatatableServer) =>
        a.descripcion_ambiente.localeCompare(b.descripcion_ambiente),
      showSort: true,
    },
    {
      name: 'Docker',
  ...
  getData(): void {
    this.isLoading = true;
    this.serviceServer.getAllServers().subscribe((data: any) => {
      this.isLoading = false;
      if (data.ok != undefined && data.ok == false) {
        this.message.error('¡Ups! Hubo un error al obtener los servidores', {
          nzDuration: 2500,
        });
        return;
      }
  ...
  changeState(dataChange: DatatableServer): void {
    ...
    this.serviceServer.changeState(dataChange.id).subscribe((data: any) => {
      ...
      if (data.ok != undefined && data.ok == false) {
        this.message.error(
          '<b>¡Ups!</b> Hubo un error al eliminar el servidor',
          { nzDuration: 2500 }
        );
  ...
  submitForm(): void {
    ...
    const { docker, nombre_servidor, ambiente, ip, sistema_operativo } =
    ...
    if (this.edit) {
      this.isSaving = true;
      this.serviceServer.editServer(this.dataForm).subscribe((data: any) => {
        this.isSaving = false;
        if (data.ok != undefined && data.ok == false) {
          this.message.error(
            '<b>¡Ups!</b> Hubo un error al editar el servidor',
            { nzDuration: 2500 }
          );
          return;
        }
      ...
      this.isSaving = true;
      this.serviceServer.saveServer(this.dataForm).subscribe((data: any) => {
        this.isSaving = false;
        if (data.ok != undefined && data.ok == false) {
        ...
        this.message.success(
          '<b>¡Execelente!</b> Se guardo el motor con exito',
          { nzDuration: 2500 }
        );
        this.getData();
        this.drawerRef.close();
  ...
  onSearchEnviroment(value: string): void {
    ...
    if (!value) {
      ...
      this.optionListEnviroments = this.allEnviroments.filter(
        ({ descripcion }) =>
