...
import {
  ...
  DatatableEngine,
  FormEngine,
} from '../../../interface/engine.interface';
...
export class EngineComponent {
  @ViewChild('drawerEngine', { static: false })
  drawerTemplate?: TemplateRef<drawerTemplate>;
  @ViewChild('footerEngine', { static: false })
  ...
  validateForm: FormGroup<FormEngine> = this.fb.group({
  ...
  listOfColumns: ColumnItemEngine[] = [
    {
      name: 'id',
      sortOrder: null,
      sortFn: (a: DatatableEngine, b: DatatableEngine) =>
  ...
  getData(): void {
    this.isLoading = true;
    this.serviceEngine.getAllEngines().subscribe((data: any) => {
      this.isLoading = false;
      if (data.ok != undefined && data.ok == false) {
        this.message.error('¡Ups! Hubo un error al obtener los motores', {
          nzDuration: 2500,
        });
        return;
      }
  ...
  changeState(dataChange: DatatableEngine): void {
    ...
    this.serviceEngine.changeState(dataChange.id).subscribe((data: any) => {
      ...
      if (data.ok != undefined && data.ok == false) {
        this.message.error('<b>¡Ups!</b> Hubo un error al eliminar el motor', {
          nzDuration: 2500,
        });
  ...
  submitForm(): void {
    ...
    if (this.edit) {
      ...
      this.serviceEngine.editEngines(this.dataForm).subscribe((data: any) => {
        this.isSaving = false;
        if (data.ok != undefined && data.ok == false) {
          this.message.error('<b>¡Ups!</b> Hubo un error al editar el motor', {
            nzDuration: 2500,
          });
          return;
        }
        ...
        this.message.success(
          '<b>¡Excelente!</b> Se actualizo el motor con éxito',
          { nzDuration: 2500 }
        );
        ...
        this.listOfData = this.listOfData.map((data) => {
          return data.id == this.dataForm.id
            ? { ...data, descripcion: this.dataForm.descripcion }
            : data;
        });
        this.dataForm = {
          id: '',
      ...
      this.isSaving = true;
      this.serviceEngine.saveEngine(descripcion).subscribe((data: any) => {
        this.isSaving = false;
        if (data.ok != undefined && data.ok == false) {
          this.message.error('<b>¡Ups!</b> Hubo un error al guardar el motor', {
            nzDuration: 2500,
          });
          return;
        }
        ...
        this.message.success(
          '<b>¡Excelente!</b> Se guardo el motor con exito',
          { nzDuration: 2500 }
        );
        this.getData();
        this.drawerRef.close();
  ...
  close(): void {
    this.drawerRef.close();
    this.dataForm = this.globalService.createDefaultObject<DatatableEngine>(
      this.dataForm
    );
  }
