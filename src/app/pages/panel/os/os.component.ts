...
import { ColumnItemOs, DatatableOs, FormOs } from '../../../interface/os.interface';
...
export class OsComponent {
  @ViewChild('drawerOs', { static: false }) drawerTemplate?: TemplateRef<drawerTemplate>
  @ViewChild('footerOs', { static: false }) drawerFooter?: TemplateRef<DrawerFooter>
  ...
  listOfColumns: ColumnItemOs[] = [
    {
      name: 'id',
      sortOrder: null,
      sortFn: (a: DatatableOs, b: DatatableOs)=> JSON.stringify(a.id).localeCompare(JSON.stringify(b.id)),
      showSort: true
    },
    {
      name: 'Sistema Operativo',
      sortOrder: null,
      sortFn: (a: DatatableOs, b: DatatableOs)=> a.nombre.localeCompare(b.nombre),
      showSort: true
    },
    {
      name: 'Version',
      sortOrder: null,
      sortFn: (a: DatatableOs, b: DatatableOs) => a.version.localeCompare(b.version),
      showSort: true
    },
    {
      name: 'Acciones',
  ...
  submitForm(): void {
    ...
    if (this.edit) {
      ...
      this.serviceOs.editOs(this.dataForm)
        .subscribe((data: any) => {
          this.isSaving = false
          if (data.ok != undefined && data.ok == false) {
      ...
      this.isSaving = true
      this.serviceOs.saveOs(nombre, version)
        .subscribe((data: any) => {
          this.isSaving = false
          if (data.ok != undefined && data.ok == false) {
          ...
          this.message.success('<b>¡Excelente!</b> Se guardo el sistema operativo con éxito', { nzDuration: 2500 })
          this.getData()
          this.drawerRef.close()
        })
    }
  ...
  close(): void {
    this.drawerRef.close()
    this.dataForm = this.globalService.createDefaultObject<DatatableOs>(this.dataForm)
  }
  ...
  search(): void{
    this.listOfData = this.listOfDataCopy.filter(({nombre, version}: DatatableOs)=> {
      return nombre.toLocaleUpperCase().includes(this.searchValue.toLocaleUpperCase()) ||
      version.toLocaleUpperCase().includes(this.searchValue.toLocaleUpperCase())
    });
  }
}
