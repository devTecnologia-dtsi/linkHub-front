...
export class languageComponent {
  @ViewChild('drawerLanguage', { static: false })
  drawerTemplate?: TemplateRef<drawerTemplate>;
  @ViewChild('footerLanguage', { static: false })
  ...
  listOfColumns: ColumnItemLanguage[] = [
    {
      name: 'id',
      sortOrder: null,
      sortFn: (a: DataLanguage, b: DataLanguage) =>
    ...
    },
    {
      ...
      sortOrder: null,
      sortFn: (a: DataLanguage, b: DataLanguage) =>
        JSON.stringify(a.nombre).localeCompare(JSON.stringify(b.nombre)),
      showSort: true,
    },
    {
      name: 'Acciones',
  ...
  getData(): void {
    ...
    this.languageService.getAllLanguage().subscribe((data: any) => {
      ...
      if (data.ok != undefined && data.ok == false) {
        this.message.error('¡Ups! Hubo un error al obtener los lenguajes', {
          nzDuration: 2500,
        });
        return;
      }
  ...
  changeState(dataChange: DataTableLanguage): void {
    ...
    this.languageService.changeState(dataChange.id).subscribe((data: any) => {
      ...
      if (data.ok != undefined && data.ok == false) {
        this.message.error(
          '<b>¡Ups!</b> Hubo un error al eliminar el lenguaje',
          { nzDuration: 2500 }
        );
  ...
  submitForm(): void {
    ...
    if (this.edit) {
      ...
      this.languageService
        .editLanguage(this.dataForm)
        .subscribe((data: any) => {
          this.isSaving = false;
          if (data.ok != undefined && data.ok == false) {
            this.message.error(
              '<b>¡Ups!</b> Hubo un error al editar el lenguaje',
              { nzDuration: 2500 }
            );
            return;
          }
          ...
          this.message.success(
            '<b>¡Excelente!</b> Se actualizo el lenguaje con éxito',
            { nzDuration: 2500 }
          );
      ...
      this.isSaving = true;
      this.languageService
        .saveLanguage(this.dataForm)
        .subscribe((data: any) => {
          this.isSaving = false;
          if (data.ok != undefined && data.ok == false) {
            this.message.error(
              '<b>¡Ups!</b> Hubo un error al guardar el lenguaje',
              { nzDuration: 2500 }
            );
            return;
          }
          ...
          this.message.success(
            '<b>¡Excelente!</b> Se guardo el lenguaje con éxito',
            { nzDuration: 2500 }
          );
          this.getData();
          this.dataForm = {
  ...
  close(): void {
    this.drawerRef.close();
    this.dataForm = this.globalService.createDefaultObject<DataLanguage>(
      this.dataForm
    );
  }
