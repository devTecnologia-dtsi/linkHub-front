...
import {
  ColumnItemRoleProject,
  DataRoleProject,
  DatatableRoleProject,
  FormRoleProject,
} from '../../../interface/roleproject.interface';
...
export class RoleProjectComponent {
  @ViewChild('drawerRoleProject', { static: false })
  drawerTemplate?: TemplateRef<drawerTemplate>;
  @ViewChild('footerRoleProject', { static: false })
  ...
  validateForm: FormGroup<FormRoleProject> = this.fb.group({
  ...
  listOfColumns: ColumnItemRoleProject[] = [
    {
      name: 'id',
      sortOrder: null,
      sortFn: (a: DatatableRoleProject, b: DatatableRoleProject) =>
  ...
  getData(): void {
    this.isLoading = true;
    this.serviceRoleProject.getAllRoleProjects().subscribe((data: any) => {
    ...
    if (this.edit) {
      ...
      this.serviceRoleProject
        ...
        .subscribe((data: any) => {
          ...
          this.message.success(
            '<b>¡Excelente!</b> Se actualizo el rol con éxito',
            { nzDuration: 2500 }
          );
      ...
      this.serviceRoleProject
        ...
        .subscribe((data: any) => {
          ...
          this.message.success(
            '<b>¡Excelente!</b> Se guardo el rol con éxito',
            { nzDuration: 2500 }
          );
          this.getData();
          this.drawerRef.close();
  ...
  close(): void {
    this.drawerRef.close();
    this.dataForm =
      this.globalService.createDefaultObject<DatatableRoleProject>({
        descripcion: '',
        id: '',
        activo: false,
        loading: false,
  ...
  search(): void {
    this.listOfData = this.listOfDataCopy.filter(
      ({ descripcion }: DatatableRoleProject) => {
        return descripcion
          .toLocaleUpperCase()
