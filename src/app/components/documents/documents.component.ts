...
// interfaces
import { DocumentosFrontend, DocumentosFrontendForm } from '../../interface/frontend.interface';
...
export class DocumentsComponent {
  ...
  editCacheTableDocument: { [key: string]: { edit: boolean; data: DocumentosFrontend } };
  ...
  submitFormDocument(): void {
    ...
    this.document = [...this.document, {
      ...
      nota: nota ?? "",
      ubicacion: ubicacion ?? "",
      nombre_documento: this.getNameByIdDocument(JSON.stringify(id_documento))
    }]
    this.editCacheTableDocument = {
      ...this.editCacheTableDocument,
  ...
  deleteDocument(tableIndex: number): void {
    ...
    this.document = this.document.filter((document, index) => index != tableIndex)
    this.resetIndexCache()
    this.documentChange.emit(this.document)
  }
  ...
  saveEditDocument(index: number): void {
    Object.assign(this.document[index], this.editCacheTableDocument[index].data);
  ...
  getNameByIdDocument(idDocument: string): string {
    const filteredServe = this.optionListDocument.filter(({ id }) => id == idDocument);
  ...
  onSearchDocument(value: string): void {
    if (!value) {
      ...
      this.optionListDocument = this.optionListCopyDocument.filter(({ nombre_documento }) => nombre_documento.toLocaleUpperCase().includes(value.toLocaleUpperCase()))
    }
  }
